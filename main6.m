...
%% Single ROC plot - This has been moved into a function ROCplotter
...
% fig1 = ROCplotter(X,Y);
% fig2 = ROCplotter(X2,Y2);
% fig3 = ROCplotter(X3,Y3);
% fig4 = ROCplotter(X4,Y4);
% fig5 = ROCplotter(X5,Y5);
% fig6 = ROCplotter(X6,Y6);
aucs = [mean(AUC),mean(AUC2),mean(AUC3),mean(AUC4),mean(AUC5),mean(AUC6)];
% aucs3 should be approximately the same as aucs
[fig8,aucs3] = ROCplotter2(X,X2,X3,X4,X5,X6,Y,Y2,Y3,Y4,Y5,Y6,'mean');
%% Save figures
cd ./plotsAndFigures
% saveas(fig1,'ROC_NTSvsM','epsc')
% saveas(fig2,'ROC_NTSvsSI','epsc')
% saveas(fig3,'ROC_NTSvsSInt','epsc')
% saveas(fig4,'ROC_MvsSI','epsc')
% saveas(fig5,'ROC_MvsSInt','epsc')
% saveas(fig6,'ROC_SIvsSInt','epsc')
saveas(fig8,'ROC_means','epsc')   % Subject mean ROC median cross subject
cd ..